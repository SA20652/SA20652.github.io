...
async def handle_successful_payment(message: types.Message):
    """Обработчик реальной оплаты Stars"""
    user_id = message.from_user.id
...
async def process_pre_checkout(query: types.PreCheckoutQuery):
    await query.answer(ok=True, error_message="Всё хорошо!")

# --- Функции ---
def verify_init_data(init_data):
    print('[AUTH] initData received:', bool(init_data), 'length:', len(init_data or ''), flush=True)
    if not init_data:
    ...
    if not hmac.compare_digest(calc, received):
        print('[AUTH] hash check FAILED', flush=True)
        raise ValueError('Недействительный Telegram initData')
    print('[AUTH] hash check OK', flush=True)
    ...
    print('[AUTH] telegram id:', user.get('id'), flush=True)
    if not user.get('id'):
...
def api_me():
    try:
        ...
        print('[AUTH] /api/me called', flush=True)
        tg=verify_init_data(payload.get('initData',''))
        print('[AUTH] creating/finding user:', tg.get('id'), flush=True)
        database.create_user(tg['id'], tg.get('username') or tg.get('first_name') or str(tg['id']))
        ...
        print(f"[AUTH] api_me error: {e}", flush=True)
...
if dp:
    ...
    async def successful_payment_handler(message: types.Message):
        try:
            ...
            # Покупка Premium 30 дней
            if payload.startswith('premium_'):
                database.create_user(uid, message.from_user.username or message.from_user.first_name or str(uid))
                database.grant_premium(uid, 30)
                await message.answer(
                    '⭐ Premium успешно активирован на 30 дней!\n\n'
                    'Доступно:\n'
                    '• x2 ежедневный бонус\n'
                    '• статус ⭐ Premium в профиле\n'
                    '• специальный значок Premium\n'
                    'Спасибо за поддержку GDPLAY!'
                )
                print(f'✅ Premium выдан пользователю {uid}')
                return

            print(f'⚠️ Неизвестный платёж: {payload}')
        except Exception as e:
            print(f'❌ Ошибка выдачи оплаты: {e}')
