...
CREATE TABLE IF NOT EXISTS users(
        ...
        # Сейф (vault_balance): отдельная от игрового баланса «неприкосновенная»
        # копилка. Деньги, отложенные сюда, никогда не участвуют в открытии
        # кейсов/апгрейде/батлах/переводах — их можно только положить или
        # забрать обратно на баланс. Единственное, что их обнуляет —
        # полный сброс аккаунта администратором (см. reset_progress).
        try:
...
def register_referral(referrer_id, referred_id, reward=7500):
    ...
    with db() as c:
        ...
        if c.execute('SELECT 1 FROM referrals WHERE referred_id=?',(referred_id,)).fetchone(): return {'created':False,'reason':'already'}
        day=c.execute("SELECT COUNT(*) c FROM referrals WHERE referrer_id=? AND created_at>=datetime('now','-1 day')",(referrer_id,)).fetchone()['c']
        suspicious=1 if int(day or 0)>=10 else 0
        ...
        return {'created':True,'reward':0,'pending':True}
...
def upgrade_key_atomic(user_id, key_id, target_name, target_rarity, target_value, stake=0):
    ...
    with db() as c:
        ...
        effective_chance = upgrade_effective_chance(chance, fail_streak)
        protected = effective_chance > chance
