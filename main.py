...
def claim_bonus_12h(user_id, premium=False):
    ...
    with _BONUS_12H_LOCK:
        ...
        if last and now - last > 24 * 60 * 60:
            # reset only cooldown chain, not total reward progress
        ...
        if streak_day <= 0:
            ...
            streak_day = min(14, streak_day + 1)
            claims_in_day = 0
        total_claims = min(MAX_CLAIMS, total_claims)
        reward = int(rewards[min(total_claims, len(rewards)-1)])
        if premium:
            reward *= 2
        ...
        return {
            ...
            'claims_in_day': claims_in_day,
            'total_claims': total_claims + 1, 'last_daily': int(now * 1000),
...
def api_daily_status():
    try:
        ...
        last = float(row.get('last_claim', 0) or 0)
        next_at = float(row.get('next_claim_at', 0) or 0)
        streak_day = min(14, max(0, int(row.get('streak_day', 0) or 0)))
        ...
        rewards = [1000,1500,2000,2500,3000,4000,5000,6000,7000,8000,9000,10000,12000,15000]
