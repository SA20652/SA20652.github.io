...
def upgrade_effective_chance(base_chance, fail_streak=0):
    """Return the exact chance used by the upgrade roll.

    There is intentionally no hidden soft-pity or streak modifier: the
    displayed percentage, green wheel sector, and server roll all use the
    same probability.
    """
    return max(0.0, min(95.0, float(base_chance or 0)))
