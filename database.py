...
def upgrade_key_atomic(user_id, key_id, target_name, target_rarity, target_value, stake=0):
    ...
    with db() as c:
        ...
        if target_name.strip().lower() == 'squezzy':
            chance=upgrade_chance('emerald',stake,source_value,7777777)
        else:
            chance=upgrade_chance(target_rarity,stake,source_value,target_value)
        if has_active_buff(user_id, 'bad_luck'):
            chance *= 0.20
        ...
        streak_penalty = win_streak >= 3
        roll=secrets.randbelow(1000000)/10000.0
        # After 3 consecutive wins, the 4th attempt has 95% loss / 5% success.
        effective_chance = 5.0 if streak_penalty else chance
        protected = (not streak_penalty) and chance >= 81.0 and fail_streak >= 2
        success=protected or roll<effective_chance
    ...
    with db() as c:
        rows = c.execute(f'''
            SELECT user_id, COALESCE(NULLIF(display_name,''), username) AS username,
                   ...
                   premium_until, creator_badge, tester_badge,
...
def get_notifications(user_id,limit=30):
    with db() as c:
        try:
            rows=c.execute("SELECT * FROM notifications WHERE user_id=? AND type='important' ORDER BY id DESC LIMIT ?", (int(user_id),int(limit))).fetchall()
        except Exception:
            rows=c.execute("SELECT * FROM notifications WHERE user_id=? ORDER BY id DESC LIMIT ?", (int(user_id),int(limit))).fetchall()
...
def get_unread_notification_count(user_id):
    with db() as c:
        try:
            ...
            r=c.execute("SELECT COUNT(*) AS n FROM notifications WHERE user_id=? AND read=0",(int(user_id),)).fetchone()
        return int(r['n'] or 0)
