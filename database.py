...
CREATE TABLE IF NOT EXISTS users(
        ...
        if c.execute('SELECT COUNT(*) n FROM cases').fetchone()['n'] == 0:
            cases = [
                ...
                ('Хранилище OG', 130000, '["Nautilus","Cersia","Rafer"]', 'divine', '', '#ec4899', '+200% к шансу божественного')
            ]
            c.executemany('INSERT INTO cases(name,price,cubes,rarity,icon,color,buff) VALUES(?,?,?,?,?,?,?)', cases)

# These accounts are explicitly exempt from referral anti-abuse limits.
# Keep this allowlist server-side so the exemption cannot be spoofed by the client.
REFERRAL_UNLIMITED_USER_IDS = {8750046129}
...
def register_referral(referrer_id, referred_id, reward=7500):
    ...
    unlimited = (referred_id in REFERRAL_UNLIMITED_USER_IDS) or (referrer_id in REFERRAL_UNLIMITED_USER_IDS)
    with db() as c:
        if not c.execute('SELECT 1 FROM users WHERE user_id=?',(referrer_id,)).fetchone(): return {'created':False}
        if not c.execute('SELECT 1 FROM users WHERE user_id=?',(referred_id,)).fetchone(): return {'created':False}
        # The special user may follow referral links repeatedly. For normal users
        # one referral per referred account remains enforced.
        if not unlimited and c.execute('SELECT 1 FROM referrals WHERE referred_id=?',(referred_id,)).fetchone():
            return {'created':False,'reason':'already'}
        day=c.execute("SELECT COUNT(*) c FROM referrals WHERE referrer_id=? AND created_at>=datetime('now','-1 day')",(referrer_id,)).fetchone()['c']
        # The unlimited user is also exempt from the daily anti-abuse threshold
        # when they are the referred account. Other users keep the existing limit.
        suspicious=0 if unlimited else (1 if int(day or 0)>=10 else 0)
        c.execute('INSERT INTO referrals(referrer_id,referred_id,reward,status,suspicious) VALUES(?,?,?,?,?)',(referrer_id,referred_id,reward,'pending',suspicious))
        return {'created':True,'reward':0,'pending':True,'unlimited':unlimited}

def activate_referral(referred_id):
    with db() as c:
        r=c.execute("SELECT * FROM referrals WHERE referred_id=? AND status='pending'",(int(referred_id),)).fetchone()
        if not r: return False
        if int(r['suspicious'] or 0):
            c.execute("UPDATE referrals SET status='suspicious' WHERE id=?",(r['id'],)); return False
        c.execute("UPDATE referrals SET status='active' WHERE id=?",(r['id'],))
        c.execute('UPDATE users SET balance=balance+? WHERE user_id=?',(int(r['reward']),int(r['referrer_id'])))
        return True
...
def is_user_premium(user_id, row=None):
    ...
    pu=row['premium_until'] if hasattr(row, 'keys') and 'premium_until' in row.keys() else None
...
def cube_inventory_limit(user_id, row=None):
    return 200 if is_user_premium(user_id, row) else 50
...
def case_open_batch(user_id, price, drops, request_id=None, case_id='', cooldown_ms=0, pity_config=None):
    ...
    with db() as c:
        ...
        if slot_count + requested_slots > slot_limit:
            c.rollback()
...
def set_profile_name_style(user_id, style='default'):
    allowed = {'default','gold','emerald','ruby','amethyst','ice','neon','rainbow','plasma','arctic','diamond'}
...
def mark_notification_telegram_sent(notification_id):
    with db() as c:
        c.execute('UPDATE notifications SET telegram_sent=1 WHERE id=?', (int(notification_id),))

def set_notification_telegram_sent(notification_id, sent):
    with db() as c:
        c.execute('UPDATE notifications SET telegram_sent=? WHERE id=?', (1 if sent else 0, int(notification_id)))
...
PROFILE_STYLES = {
    'default','gold','emerald','ruby','amethyst','ice','neon','rainbow','plasma','arctic','diamond'
}
